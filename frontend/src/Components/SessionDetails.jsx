...
const SessionDetails = () => {
  ...
  // Utilisez `useSessions` pour accéder à `sessions` et autres fonctions
  const { sessions, removeSession, removeOrderFromSession } = useSessions();
  
  const [selectedSessionId, setSelectedSessionId] = useState(sessionId || selectedPickingSessionId || "");
  ...
  const handleDeleteSessionClick = () => {
    if (window.confirm(`Êtes-vous sûr de vouloir supprimer la session "${currentSession?.name}" ?`)) {
  ...
  const handleRemoveOrderFromSession = (orderId) => {
    ...
    const isConfirmed = window.confirm(`Supprimer la commande "${order?.name}" de la session "${currentSession?.name}" ?`);
    if (isConfirmed) {
  ...
  return (
    <div>
      <div className="orders-title">
        ...
        <select className="sessionSelect" value={selectedSessionId} onChange={handleSessionChange}>
